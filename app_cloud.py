...
// Print — שולח base64 כדי לעקוף WAF
...
function saveH(html){
  try{
    ...
    const t=html.match(/<title[^>]*>([^<]+)/i)?.[1]?.trim()||html.match(/<h1[^>]*>([^<]+)/i)?.[1]?.replace(/<[^>]+>/g,'').trim()||'מסמך';
...
function showHist(){
  const card=document.getElementById('histCard'),list=document.getElementById('histList');
  ...
  list.innerHTML=a.map((it,i)=>`<div class="hist-item" onclick="loadH(${i})"><div class="hist-dot"></div><div class="hist-title">${it.t.replace(/</g,'&lt;')}</div><div class="hist-meta">${f(it.ts)}</div></div>`).join('');
}
...
function loadH(i){
  ...
  upd(a[i].html);hideHist();toast('מסמך נטען','ok');
}
...
def show():
    # קבלת נתונים — תומך ב-base64 וב-plain
    ...
    try:
        ...
        html = raw  # אם לא base64 — השתמש כמות שהוא
