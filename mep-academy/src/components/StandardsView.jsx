  ...
  const filtered = useMemo(() => {
    ...
    return STANDARD_DOMAINS.map((d) => ({
      ...
      items: d.items.filter(
        (it) => it.code.includes(q) || it.desc.includes(q) || d.title.includes(q)
      ),
  ...
  return (
    <div className="space-y-4">
      <header className="bg-ink text-white rounded-2xl p-6">
        <h2 className="font-bold text-2xl mb-1">📖 ספריית התקנים</h2>
      ...
      {filtered.length === 0 && (
        <p className="text-center text-ink/60 py-8">לא נמצאו תקנים מתאימים לחיפוש.</p>
      )}
