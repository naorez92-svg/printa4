...
export default function Students({ onBookletSaved, remaining, isPro }) {
  ...
  const fetchStudents = async () => {
    const { data } = await supabase.from("children").select("*").order("grade").order("name");
    setStudents(data || []);
    setLoading(false);
  };
  ...
  const addStudent = async () => {
    ...
    await supabase.from("children").insert({
      user_id: user.id, name: form.name.trim(), grade: form.grade,
      level: form.level, special_needs: form.special_needs.trim() || null,
    });
    setSaving(false); setShowAdd(false); setForm(EMPTY); fetchStudents();
  };
  ...
  if (quickCreate) return (
    <QuickCreate student={quickCreate} onClose={() => setQuickCreate(null)}
      onSaved={() => { setQuickCreate(null); onBookletSaved?.(); }}
      remaining={remaining} isPro={isPro} />
  );

  if (history) return (
    <StudentHistory student={history} onBack={() => setHistory(null)}
      remaining={remaining} isPro={isPro} />
  );
    ...
    <div className="space-y-4">
      ...
      {showAdd && (
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-ink/5 space-y-3">
          ...
          <input autoFocus className="w-full border border-ink/20 rounded-xl p-3 outline-none focus:border-magic text-right bg-canvas/50"
            placeholder="שם התלמיד/ה *" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} />
          <select className="w-full border border-ink/20 rounded-xl p-3 bg-canvas/50 text-right outline-none focus:border-magic"
            value={form.grade} onChange={e => setForm(p => ({ ...p, grade: e.target.value }))}>
            {GRADES.map(g => <option key={g}>{g}</option>)}
          </select>
          <div className="flex gap-2">
            {LEVELS.map(([v, t]) => (
              <button key={v} onClick={() => setForm(p => ({ ...p, level: v }))}
                className={`flex-1 rounded-xl p-2 text-sm font-medium border transition-colors ${form.level === v ? "bg-magic text-white border-magic shadow-sm" : "bg-canvas/50 border-ink/15 text-ink/60 hover:border-magic/50"}`}>{t}</button>
            ))}
          </div>
          <textarea className="w-full border border-ink/20 rounded-xl p-3 outline-none focus:border-magic text-right resize-none bg-canvas/50 text-sm"
            placeholder="הערות (אופציונלי) — קשיים, חוזקות, הנחיות מיוחדות..." rows={2}
            value={form.special_needs} onChange={e => setForm(p => ({ ...p, special_needs: e.target.value }))} />
          <div className="flex gap-2">
            <button onClick={addStudent} disabled={saving || !form.name.trim()}
            ...
            <button onClick={() => { setShowAdd(false); setForm(EMPTY); }}
              className="px-5 border border-ink/15 rounded-xl text-ink/50 hover:text-ink transition-colors">ביטול</button>
          </div>
        </div>
      )}
      ...
      {!loading && students.length === 0 && !showAdd && (
        <div className="bg-white rounded-2xl p-8 shadow-sm border border-ink/5 text-center space-y-4">
          ...
          <button onClick={() => setShowAdd(true)} className="bg-magic text-white rounded-xl px-5 py-2.5 font-medium hover:opacity-90">
            + הוסף תלמיד ראשון
          </button>
        </div>
      )}
      ...
      {!loading && Object.entries(byGrade).map(([grade, list]) => (
        <div key={grade} className="space-y-2">
          ...
          {list.map(student => (
            <div key={student.id} className="bg-white rounded-2xl px-4 py-3.5 shadow-sm border border-ink/5 flex items-center gap-3">
