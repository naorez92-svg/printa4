...
=== כלל ברזל: כל חוברת היא מסיון (QUEST) חוויתי! ===
אסור לייצר "דף תרגילים" סטנדרטי — כל חוברת היא הרפתקה עם עלילה ומתח!
בחר מסיון שמתאים לעולם התוכן ולנושא הלימוד:
...
בעמוד 1 (שער): הצג "קובץ מסיון סודי" — תיבה מיוחדת עם סיפור הרקע ומטרת ההרפתקה.
בכל עמוד תרגיל: שורת התקדמות בצבע "⚡ שלב X מתוך Y — [משפט מסיפור ההרפתקה]"
בעמוד האחרון: 🏆 "כבשת את המסיון! [פרס סמלי לפי עולם התוכן]"
...
<div class="bg-purple-50 border-2 border-purple-300 rounded-xl p-3 mb-2">
  <p class="text-xs font-bold text-purple-800 mb-1">⚔️ [כותרת השלב בסיפור המסיון — מרגשת ותלויית מתח!]</p>
...
עמוד 1 — שער + קובץ מסיון:
  ...
  • קובץ מסיון סודי — תיבה מיוחדת עם גבול מנוקד/מקוטע, בגוון כהה:
    <div style="border:2px dashed #6b7280;border-radius:12px;padding:10px;background:#f9fafb;margin-top:8px">
      <p style="font-size:11px;font-weight:700;color:#374151;margin:0 0 4px">📋 קובץ מסיון — סודי ביותר</p>
      <p style="font-size:10px;color:#6b7280;margin:0 0 2px">🎯 <strong>המשימה:</strong> [תיאור מסיון בשורה אחת מרגשת]</p>
...
עמוד אחרון — ניצחון + רפלקציה:
  • 🏆 כותרת ניצחון מרגשת: "[שם], כבשת את המסיון!" + תגמול סמלי לפי עולם התוכן
