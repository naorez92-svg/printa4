...
חובה: בכל עמוד עם תרגילי חשבון — שלב לפחות מחשה אחת מהסוג המתאים (ציר לחיבור/חיסור, מודל עמודות לבעיות מילוליות, עיגול/פס שברים לשברים ועשרוניים). התאם צבעים לעולם התוכן כשאפשר.
...
חובה לכל עמוד: גוון את הפריסה — שלב כרטיסים ברוחב מלא, רשתות, ופאנלים זה-לצד-זה. לעולם לא רק תיבות+קווים בשורה.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const maxTokens = fast
      ...
      : Math.min(64000, Math.max(8000, effPages * 9000)); // richer magazine design = more HTML; extra headroom prevents mid-booklet truncation
